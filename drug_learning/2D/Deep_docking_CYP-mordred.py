...
features_shared = pd.read_csv(os.path.join("features", f"shared_set_features_{FINGERPRINT}.csv"))
features_only_2c9 = pd.read_csv(os.path.join("features", f"only_2c9_set_features_{FINGERPRINT}.csv"))
features_only_3a4 = pd.read_csv(os.path.join("features", f"only_3a4_set_features_{FINGERPRINT}.csv"))

features_shared[features_shared.columns] = preprocessing.scale(features_shared[features_shared.columns])
features_only_2c9[features_only_2c9.columns] = preprocessing.scale(features_only_2c9[features_only_2c9.columns])
features_only_3a4[features_only_3a4.columns] = preprocessing.scale(features_only_3a4[features_only_3a4.columns])
...
if set_params == "extended":
    ...
    HP_NEURONS = hp.HParam("neurons", hp.Discrete([i for i in range(10,800,50)]))
    HP_DROPOUT = hp.HParam("dropout", hp.RealInterval(0.2, 0.5))
    HP_OPTIMIZER = hp.HParam('optimizer', hp.Discrete(['adam', 'sgd','RMSprop']))
    HP_L2 = hp.HParam('l2 regularizer', hp.RealInterval(.001,.01))
    HP_LR = hp.HParam("learning_rate", hp.Discrete([0.001, 0.01, 0.1, 1.0, 10.0]))
    logs_path = f"hyperparameters_tunning/{FINGERPRINT}/logs/hparam_tuning"
    ...
    HP_LR = hp.HParam("learning_rate", hp.Discrete([0.001, 0.01, 0.1, 1.0, 10.0]))
    logs_path = f"hyperparameters_tunning/{FINGERPRINT}/logs_2/hparam_tuning"
